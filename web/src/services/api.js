...
api.interceptors.response.use(
  ...
  (err) => {
    ...
    // Sin backend: modo demo con datos reales de Barrancas
    if (!err.response) {
      const mock = getMockForRequest(err.config || {});
      if (mock !== null && mock !== undefined) {
        window.__RADAR_DEMO_MODE__ = true;
        return Promise.resolve({ data: mock, status: 200, _demo: true });
      }
      console.warn('[RADAR] API no disponible y sin mock para:', err.config?.url);
    }
