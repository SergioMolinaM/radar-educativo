...
@app.post("/admin/migrate-extra-tables")
def migrate_extra_tables(secret: str = ""):
    """Load egresados + SAE results into Render DB."""
    if secret != os.getenv("MIGRATE_SECRET", "radar-migrate-2026"):
        from fastapi import HTTPException
        raise HTTPException(status_code=403, detail="Forbidden")
    ...
    from api.db.connection import get_cursor
    import pathlib
    sql_file = pathlib.Path(__file__).parent / "scripts" / "extra_tables_sync.sql"
